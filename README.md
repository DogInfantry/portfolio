...
- **The merged index**: `data/work.ts` flattens projects and research into one `WorkItem[]` for `/work`, `/research` and the home page, and defines the `groups` the index is split into (decks and case studies, published research, live apps and code). It derives everything from the two files above, so nothing is maintained twice. `featuredSlugs` picks the three items the home page leads with.
